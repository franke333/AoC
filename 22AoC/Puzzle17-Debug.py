...
def printLevel(level,piece=None,x=0,y=0):
    for i in range(len(level)):
        line = level[-(i+1)]
        yy = len(level)-i-1
        string = ""
        for k,j in enumerate(line):
            if piece and yy >= y and yy < y+piece.height:
                if k >= x and k < x+piece.width and piece.data[yy-y][k-x] == 1:
                        string += '@'
                else:
                    string += '.' if j == 0 else '#'
            else:
