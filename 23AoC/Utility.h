...
#define ASSERT(x) if(!(x)) { std::cout << "Assertion failed at line: " << __LINE__ << " in file: " << __FILE__ << std::endl; return; }
// assert with line of code and file and custom message
#define ASSERTM(x, m) if(!(x)) { std::cout << "Assertion failed at line: " << __LINE__ << " in file: " << __FILE__ << " with message: " << m << std::endl; return; }
// assert with line of code and file and custom message and return value
#define ASSERTMR(x, m, r) if(!(x)) { std::cout << "Assertion failed at line: " << __LINE__ << " in file: " << __FILE__ << " with message: " << m << std::endl; return r; }
